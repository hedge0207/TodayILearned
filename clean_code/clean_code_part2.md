...
- 자료 추상화
  ...
  - 인터페이스 `Point`에도 `getX()`, `getY()` 메서드가 있는데 구현을 외부로 노출한 것 아닌가?
    - 사용자는 `getX()`를 호출할 때, 클래스 내부에 `double x`라는 변수가 있는지, 아니면 극좌표(`r`, `theta`)만 가지고 있다가 계산해서 값을 내주는지 알 필요가 없다.
    - 즉, 사용자가 "이 객체가 데이터를 어떻게 들고 있는지" 신경 쓰지 않고 "좌표"라는 개념 자체를 다루게 만들기 때문에 구현이 감춰졌다고 볼 수 있다.
...
- 자료/객체 비대칭
  ...
  - 절차적인 도형 클래스
    ...
    - 그러나 새 도형을 추가해야 할 경우 `Geometry` 클래스에 속한 함수를 모두 고쳐야하지만 도형 클래스는 변경하지 않아도 된다.
...
- 디미터 법칙
  ...
  ```
    ...
    - 나누어진 코드도 디미터 법칙을 위반하는가?
      ...
      - 코드를 아래와 같이 자료구조로 구현했다면 디미터 법칙을 거론할 필요가 없었을 것이다.
...
- null을 전달하지 마라
  ...
  - 메서드가 null을 반환하는 방식도 나쁘지만 메서드에 null을 전달하는 방식은 더 나쁘다.
