...
- 의미 있는 맥락을 추가하라
  ...
  - 스스로 의미가 분명한 이름이 있지만, 대다수 이름은 그렇지 못하다.
    - 이름 그 자체로는 충분한 의미를 제공하지 못하는 경우에는 이름을 클래스, 혹은 함수 내의 맥락에서 해석되도록 할 수 있다.
    
  - 나쁜 예시
    - 아래 변수명들을 끝까지 모두 읽어보면 학생에 대한 정보들을 저장한 변수들라는 것을 알 수 있다.
...
- 클래스명과 메서드명
  ...
  - 클래스명은 명사나 명사구가 적합하다.
    ...
    - Manager, Processor, Data, Info 등과 같은 단어는 이름만 보고 역할을 유추할 수 없으므로 피해야한다.
...
- 다형성을 활용하기
  ...
  - 위 함수에는 몇 가지 문제가 있다.
    ...
    - 셋째로 SRP(Single Responseibility Principle)를 위반한다(코드를 변경할 이유가 여럿이기 때문이다).
...
- 구조적 프로그래밍
  ...
  - 다익스트라는 모든 함수와 함수 내 모든 블록에 입구와 출구가 하나만 존재해야 한다고 말했다.
    ...
    - go to는 절대로 안된다.
  - 근본적인 목적은 함수의 흐름을 이해하기 쉽게 만드는 것이다.
    - 따라서 함수의 흐름을 이해하는데 방해가 되지 않는다면 여러 개의 return문(e.g. early return)이나 continue, break를 사용해도 괜찮다.
