...
- Vector quantization
  ...
  - 고차원 벡터를 더 적은 비트로 압축하여 메모리 사용량을 줄이는 기법이다.
...
- Scalar quantization
  ...
  - 동작 방식
    - 예를 들어  -1에서 1 사이의 값을 갖는 벡터를 -128에서 127까지 표현할 수 있는 int8로 quantization한다고 가정해보자.
    - 원본 벡터 값의 범위(min~max)를 256개의 구간으로 나누고, 각 구간에 -128~127 중 하나의 정수를 라벨로 붙인다
  - 정보의 손실
...
- 선형 탐색 구현하기
  ...
  - 아래와 같이 index를 생성하고 데이터를 삽입한다.
    -  `index`를 `true`로 설정하면 knn search를 사용 가능하다.
    - 다만, 아래 예시에서는 script score query를 사용할 것이므로 false로 설정한다.
...
- Approximate k-NN search 구현하기
  ...
  - 아래와 같이 index를 생성한다.
    ...
    - `index_options.type`을 `int8_hnsw`로 설정하여 index를 생성하고, vector값들을 색인하면, vector들은 scalar-quantized HNSW graph에 색인된다.
...
- HNSW index 사용시 더 적은 문서를 탐색할수록 검색 속도는 더 느려질 수 있다.
  - 직관적으로는 더 많은 문서를 filtering하면 확인해야 하는 문서의 개수가 줄어들어 더 빠른 검색이 가능할 것 같다.
    - Lexical search에서는 후보 문서의 감소는 점수를 계산해야 하는 문서 개수의 감소를 의미하므로 후보 문서가 감소할수록 검색 속도는 더 빨라진다.
    - 그러나 HNSW index에서 문서를 filtering하여 후보군의 개수가 줄면 kNN 검색의 속도가 감소할 수 있다.
  - HNSW 그래프에서 속도에 영향을 미치는 주요한 요소는 k개의 최근접 이웃을 찾기 위해 vector 비교가 발생하는 횟수이다.
    - Filter를 통과한 문서의 개수가 적을수록 노드들 사이의 연결이 드물어져 많은 노드를 순회해야 한다.
    - 즉 filter를 통과한 문서들만을 대상으로 순회하는 것이 아니라, 순회하면서 filter를 통과하는 노드를 찾는 방식이므로 더 많은 횟수의 탐색이 발생하게 된다.
    - 직관적으로 표현하면 "결과로 쓸 수 있는 노드는 적은데, 그 노드를 찾기 위해 쓸모없는 노드를 더 많이 방문해야 한다".
  - 그럼 애초에 filtering된 노드들은 무시하면 되는 것 아닌지?
    - 일단 Lucene이 그런 방식으로 구현되지 않았다.
    - 또한 HNSW 그래프 자체가 성기게 구성되기에, filtering된 노드들을 아예 무시할 경우 더 이상 탐색을 못 하고 막히는 일이 자주 발생하게 될 것이다.
    - 예를 들어 entrypoint와 연결된 5개의 노드가 모두 filter로 걸려졌다면, 탐색은 더 이상 진행되지 못 한다.
  - Elasticsearch는 이를 해결하기 위해 Liana Patel 등이 고안한 ACORN-1 알고리즘을 적용했다.
    - ACORN-1 알고리즘은 filtering 된 노드들은 탐색 범위에서 제외하되, 이웃의 이웃 노드까지 탐색하는 방식을 사용한다.
    - 즉, 노드 하나 당 이웃 노드의 개수가 20개라면 20개의 이웃만 확인하지 않고 그 이웃노드까지 확인하여 400개의 이웃을 확인하는 방식으로 동작한다.
    - 또한 filtering 된 문서가 매우 많아 이것으로 충분하지 않을 경우 더 확장해서 탐색한다.
    - 이를 통해 filtering된 노드들은 제외하면서도 그래프가 성기게 구성된다는 HNSW 그래프의 한계도 극복할 수 있게 된다.
  - 다만 항상 ACORN-1 알고리즘을 적용하는 것은 아니다.
    - Index 전체로 보면 전체 node들 중 40%가 필터링될 경우 ACORN-1을 활성화한다(40%는 테스트를 통해 정한 수치이다).
...
- kNN 검색에 필요한 메모리
  ...
  $$
  estimated\ bytes=(1,000,000×4×16)+(1,000,000×4×1024)\\
...
- `.cfs` 파일
  - Compound File Segment 파일로 여러 개의 인덱스 구성 파일들을 하나로 합쳐놓은 파일이다.
    - 기본적으로 Elasitcsearch(Lucene)는 세그먼트당 약 10~15개의 서로 다른 확장자를 가진 파일이 생성된다.
    - 그런데, 매우 작은 파일들이 여러 개 있을 경우 이들을 모두 읽으면 file descriptor를 지나치게 많이 사용하게 된다.
    - 또한 수많은 작은 파일들을 관리하는 것보다 하나의 큰 파일을 관리하는 것이 운영체제 입장에서 오버헤드가 적다.
    - 따라서 Elasitcsearch(Lucene)는 작은 파일들을 합쳐 `.cfs`라는 하나의 파일로 관리한다.
    - 모든 파일을 `.cfs` 파일로 합치는 것은 아니고, 일정 크기 이하의 파일들만 합친다.
    - 기본값은 하나의 세그먼트를 구성하는 전체 파일 집합의 크기가 인덱스 크기의 10% 미만일 경우 cfs 파일로 관리하는 것이다.
  - `dense_vector` 필드와 관련된 `.vec`, `.veq`, `.vex` 파일도 예외 없이  `.cfs` 파일로 합쳐진다.
    - 문제는 warm up을 위해 `index.store.preload`에 `.veq`, `.vex` 파일들을 설정해 두는데, `.cfs`파일로 합쳐질 경우 warm up이 되지 않는다는 것이다.
    - 따라서 이 경우 `.cfs`파일도 `index.store.preload`에 추가하는 것을 고려해 볼 수 있다.
    - 다만, `.cfs` 파일로 합쳐졌다는 것은 파일의 크기가 그렇게 크지 않다는 것이므로 warm up 없이도 검색 속도가 빠를 가능성이 높다.
    - 또한 `.cfs`파일에는 vector와 관련된 파일만 합쳐져 있는 것은 아니기에 불필요한 내용들도 모두 메모리에 올라갈 수 있다는 것도 유념해야 한다.
