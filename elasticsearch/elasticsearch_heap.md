...
- Heap memory 크기를 정하는 규칙
  ...
  - heap 사이즈가 전체 RAM 용량의 50%를 넘지 않도록 설정하는 것이 좋다. 
    ...
    - 특히 page cache를 많이 사용하는 작업을 수행할 경우 page cache의 부족으로 성능이 떨어질 수 있다.
  ...
  - JVM은 설정된 heap 외에도 JVM overhead를 추가로 필요로 한다.
    - JVM 런타임 자체도 메모리를 쓰기 때문으로 주로 아래와 같은 곳에 사용한다.
    - 클래스 메타데이터 저장 하기 위한 Metaspace(Java 8+이상부터 PermGen이 대체)
    - JVM이 컴파일한 네이티브 코드(JIT 컴파일 코드 캐시)
    - GC 알고리즘이 관리하는 내부 자료구조
    - 스레드마다 할당되는 스택 메모리
