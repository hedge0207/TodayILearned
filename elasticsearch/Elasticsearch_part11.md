...
- 적절한 shard의 크기와 node 하나 당 적절한 shard의 개수는 Elasticsearch 사용자들 사이에서 꾸준히 논의되던 주제이다.
  ...
  - CPU core와 shard 개수의 관계
    - Elastic이 공식적으로는 "샤드당 크기(10~50GB)"를 먼저 권장하지만, CPU 활용 관점에서 자주 쓰이는 경험칙
    - 샤드 수 ≤ 노드당 CPU 코어 수: 단일 쿼리가 모든 CPU를 쓸 수 있는 상한
    - 샤드 수 ≈ CPU 코어 수의 절반: 단일 쿼리 병렬성과 동시 쿼리 처리의 균형점
    - 샤드 수 ≪ CPU 코어 수: 동시 쿼리 처리 중심 (QPS 최대화)
    - 16 CPU 컨테이너 → 샤드 2~8 범위가 합리적
