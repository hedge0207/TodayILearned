...
- Counting sort
  ...
  - 요소들의 개수를 기반으로 정렬하는 알고리즘이다.
    - 시간복잡도는 O(N)으로 퀵 정렬보다 빠르다.
  - 동작 원리
    - 배열에 등장하는 고유한 요소들의 개수를 센다.
    - 고유한 요소들을 순서대로 순회하면서 등장하는 개수만큼 새로운 배열에 추가한다.
  - 구현

  ```python
  def counting_sort(nums: list[int]) -> list[num]:
      max_ = max(nums)
      count = [0] * (max_ + 1)
      for num in nums:
          count[num] += 1
      ...
      for i in range(max_+1):
          sorted_nums += [i]*count[i]
      return sorted_nums
  ```

  - 빠른 속도로 정렬이 가능한 이유
    - 각 요소들을 비교하지 않고 개수만 세는 더 단순한 방식으로 동작한다.
    - 다만 그 대신 다른 정렬에 비해 메모리를 더 많이 사용한다.
  - O(N)이라는 매우 빠른 속도로 정렬이 가능함에도 퀵 정렬과 같이 범용적으로 사용되지 않는 이유는 아래와 같다.
    - 요소들이 정수 또는 정수로 매핑 가능한 경우에만 정렬이 가능하며, 실수는 정렬할 수 없다.
    - 만일 고유한 값의 개수가 지나치게 많을 경우, count를 필요한 메모리가 증가하며, 사용할 수 없을 수도 있다.

  - 아래와 같은 경우에 유용하게 사용할 수 있다.
    - 나이, 시험 점수와 같이 값의 범위가 한정되어 있는 경우.
    - 중복 데이터가 많은 경우
