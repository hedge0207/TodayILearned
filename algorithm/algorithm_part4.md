...
- 다익스트라 알고리즘
  ...
  - 거리가 확정된 정점들인 1, 2, 3번 정점에서 갈 수 있는 정점 중 거리가 가장 가까운 정점을 선택한다.
...
- O(V<sup>2</sup>+E)로 구현하기
  ...
  - 이제 다시 아직 확인하지 않은 D[2], D[4], D[5], D[6] 중 가장 작은 값을 가지는 정점의 값을 확정한다.
    - D[2]가 3으로 가장 작으므로 D[2]의 거리를 2로 확정한다.
    - 그 후 2번 정점에서 갈 수 있는 곳 들의 값을 갱신한다.
    - 3번, 5번 정점으로 가는 것이 가능한데, 3번 정점의 값은 이미 확정 됐으므로 5번 정점으로 가는 값만 갱신한다.
    - D[5\](∞)보다 2번 정점을 거쳐서 5번 정점으로 가는 D[2\](3) + 8이 더 작으므로 11로 값을 갱신한다.

  | 1    | 2    | 3    | 4    | 5    | 6    |
  | ---- | ---- | ---- | ---- | ---- | ---- |
  | 0    | 3    | 2    | 4    | 11   | ∞    |

  - 이제 다시 값이 확정되지 않은 D[4], D[5], D[6] 중 가장 작은 값을 가지는 정점의 값을 확정한다.
    - D[4]가 4로 가장 작으므로 D[4]의 거리를 4로 확정한다.
    - 4번 정점에서는 5번 정점으로 가는 것이 가능하다.
    - D[5]보다 D[4] + 5가 크므로 값을 갱신하지 않고 넘어간다.
  
  | 1    | 2    | 3    | 4    | 5    | 6    |
  | ---- | ---- | ---- | ---- | ---- | ---- |
  | 0    | 3    | 2    | 4    | 10   | ∞    |
  
    - 다시 값이 확정되지 않은 D[5], D[6] 중 가장 작은 값을 가지는 정점의 값을 확정한다.
      - D[5]가 가장 작으므로, D[5]의 값을 10으로 확정한다.
      - 5 번 정점에서 갈 수 있는 정점은 6번 정점밖에 없다.
      - D[6]보다 D[5] + 1이 작으므로 D[6]의 값으로 D[5] + 1로 갱신한다. 
  
  | 1    | 2    | 3    | 4    | 5    | 6    |
  | ---- | ---- | ---- | ---- | ---- | ---- |
  | 0    | 3    | 2    | 4    | 10   | 11   |
  
    - 값이 확정되지 않은 6번 정점의 값을 11로 확정한다.
...
- 우선순위 큐 활용
  ...
  - 지금까지 살펴본 과정은 다익스트라가 초창기에 고안한 아이디어로 시간 복잡도는 O(V<sup>2</sup>+E)이다.
  - 우선순위 큐를 활용하면 O(ElgV)(OElgE 라고 하기도 하는데 Big-O 관점에서 보면 둘 다 동일한 것이다)로 구현이 가능하다.
  - 구현 원리
    - 우선순위 큐에 시작점을 추가
  ...
  - 우선순위 큐에서 거리가 가장 작은 정점을 선택한다.
    - 현재 우선순위 큐에서 거리가 가장 작은 정점은 거리가 0인 1번이므로 1번을 우선순위 큐에서 뺀다.
    - 최단 거리 테이블 d에서 d[1]의 값이 0이 맞는지 확인한다(`d[1] == 0` 확인).
    - `d[1]==0`이므로 1번 정점에서 갈 수 있는 정점들을 확인한다.
    - 1번에서 갈 수 있는 정점 2, 3, 4 번 정점까지의 거리는 각 3,2,5로 모두 ∞보다 작으므로 갱신시키고 우선순위 큐에는 갱신된 거리와 정점 쌍을 넣는다.
    - 우선순위 큐: [[3,2], [2,3], [5,4]]
  ...
  - 우선순위 큐가 비어 있지 않으므로 우선순위 큐에서 거리가 가장 작은 정점을 선택한다.
    - 거리가 가장 작은 정점은 거리가 2인 3번 정점으로 3번 정점을 우선순위 큐에서 뺀다.
    - `d[3] == 2`라는 것을 확인하고 3번 정점에서 갈 수 있는 정점들을 확인한다.
    - 3번 정점에서 갈 수 있는 정점은 4번 정점 뿐으로 d[4]보다 d[3]+2가 작으므로 최단 거리 테이블에서 4의 값을 갱신하고 우선순위 큐에 (4, 4)를 넣는다.
    - 우선순위 큐:  [[3,2], [5,4], [4, 4]]
  ...
  - 우선순위 큐가 비어 있지 않으므로 우선순위 큐에서 거리가 가장 작은 정점을 선택한다.
    ...
    - 거리가 가장 작은 정점은 거리가 3인 2번 정점으로 2번 정점을 우선순위 큐에서 뺀다.
    - `d[2]==3`라는 것을 확인하고 2번 정점에서 갈 수 있는 정점들을 확인한다.
    - 2번 정점에서 갈 수 있는 정점은 3, 5번 정점이 있고 각 거리는 d[2]+2, d[2]+8이다.
    - 사실 3번 정점은 다익스트라 알고리즘의 원리 상 이미 확정이 됐지만, 우선순위 큐로 구현 할 때는 확정 여부를 따로 저장하지 않기 때문에 모두 비교해봐야한다.
    - d[2]+2는 d[3]보다 크므로 갱신하지 않고 넘어가고, d[2] + 8은 d[5]보다 작으므로 d[5]를 갱신하고 [11, 5]를 우선순위 큐에 넣는다.
  ...
  - 우선순위 큐가 비어 있지 않으므로 우선순위 큐에서 거리가 가장 작은 정점을 선택한다.
    - 거리가 가장 작은 정점은 거리가 4인 4번 정점으로 이를 우선순위 큐에서 뺀다.
    - `d[4] == 4`라는 것을 확인하고 4번 정점에서 갈 수 있는 정점들을 확인한다.
    - 4번 정점에서 갈 수 있는 정점은 5번 정점 뿐으로 d[4] + 6이 d[5]보다 작으므로 d[5]를 10으로 갱신하고 [10, 5]를 우선순위 큐에 넣는다.
    - 우선순위 큐: [[5,4], [11,5], [10, 5]]
  ...
  - 우선순위 큐가 비어 있지 않으므로 우선순위 큐에서 거리가 가장 작은 정점을 선택한다.
    - 거리가 가장 작은 정점은 거리가 5인 4번 정점으로 이를 우선순위 큐에서 뺀다.
    - 그런데 이번에는 `d[4] == 5`가 아니므로 이후 과정을 건너 뛴다.
    - 우선순위 큐: [[11, 5], [10, 5]]

  - 우선순위 큐가 비어 있지 않으므로 우선순위 큐에서 거리가 가장 작은 정점을 선택한다.
    - 거리가 가장 작은 정점은 거리가 10인 5번 정점으로 이를 우선순위 큐에서 뺀다.
    - `d[5] == 10`라는 것을 확인하고 5번 정점에서 갈 수 있는 정점들을 확인한다.
    - 5번 정점에서 갈 수 있는 정점은 6번 정점 뿐으로 d[5] + 1이 d[6]보다 작으므로 d[6]을 갱신하고 [11, 6]을 우선순위 큐에 넣는다.
    - 우선순위 큐: [[11,5], [11, 6]]
  ...
  - 우선순위 큐가 비어 있지 않으므로 우선순위 큐에서 거리가 가장 작은 정점을 선택한다.
    - 5와 6 모두 거리가 11로 같으므로 아무거나 선택하여 우선순위 큐에서 뺀다(이번에는 5를 선택).
    - `d[5] == 11`이 아니므로 이후 과정을 건너뛴다.
    - 우선순위 큐: [[11, 6]]
  - 우선순위 큐가 비어 있지 않으므로 우선순위 큐에서 거리가 가장 작은 정점을 선택한다.
    - 6번 정점이 선택되고 `d[6]==11`이지만 6번 정점에서는 갈 수 있는 곳이 없기에 갱신이 일어나지 않는다.
    - 우선순위 큐: [ ]
...
- 경로 복원
  ...
  - 1번 정점에서 시작하여 2,3,4 번 정점까지의 거리를 기록하면 최단거리 테이블은 아래와 같이 된다.
    - 우선순위 큐: [[2,3], [2,3], [5,4]]
  ...
  - 그 후 거리가 가장 작은 3번 정점을 거쳤을 때 거리가 더 줄어드는 정점은 4번 정점 뿐이므로 pre table의 4번 정점 값을 3으로 갱신한다.
  ...
  - 경로 복원 방법은 다음과 같다.
    - 1번 정점에서 6번 정점까지 가는 경로를 구하려 한다.
    - pre[6]은 5이므로 pre[5]의 값을 확인하고, pre[4]의 값은 3이므로 pre[3]의 값을 확인한다.
    - 이런 식으로 출발 정점인 1번 정점이 나올 때 까지 추적하면 경로를 알 수 있다.
...
- 구현
  ...
  - Python code
    - graph를 `defaultdict(list)`가 아니라 `defaultdict(dict)`로 구현할 경우 서로 다른 정점 사이에 복수의 간선이 있으면 덮어씌워져 처리할 수 없게 되므로, 복수의 간선이 있을 경우 `defaultdict(list)`로 구현해야 한다.
