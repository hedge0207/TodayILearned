...
- 테스트 코드 작성하기
  ...
  - 아래 코드의 테스트 코드를 작성할 것이다.
    - 세 가지 종료점의 유형중 반환 값이 있는 종료점에 대해서만 테스트할 것이다.
  ...
  - sum 함수를 테스트하기 위한 테스트 코드 작성하기
    ...
    - 또 테스트 코드는 발생하는 모든 예외를 catch하여 이후 다른 메서드의 실행에 지장이 없도록 한다(테스트 프레임워크를 사용하면 보통 프레임워크가 자동으로 처리한다).
...
- 함수 테스트하기
  ...
  - 아래와 같이 비밀번호를 검증하는 함수를 작성한다.
    ...
    - 매개변수로 전달된 규칙 함수 배열을 반복하면서 각 규칙 함수를 입력값(`password`)을 넘겨 호출한다.
  ...
  - 매개변수 `rules`로 전달되는 함수는 아래 형식으로 값을 반환해야 한다.
  ...
  - 준비 - 실행 - 검증(Arrange - Act - Assert, AAA) 패턴
...
- 함수 리팩터링 후 테스트하기
  ...
  - `verify_password()` 함수를 아래와 같이 리팩터링한다.
    ...
    - 기존 함수는 상태를 유지하지 않았지만, 이제는 `_rules`로 상태로 유지하고 있는다.
  ...
  - 검증 룰렛
    ...
    - 그렇다고 두 번째 검증 코드가 통과했는지 확인해보기 위해 첫 번째 검증 코드를 주석처리 하는 것 역시 좋은 테스트 방식이 아니다.
  ...
  - 주의 사항
    ...
    - 초기화 코드나 특정 테스트에만 필요한 코드, 모든 테스트에 영향을 미치는 코드 등 온갖 것을 다 집어넣기 때문이다.
...
- 다양한 입력 값으로 테스트하기
  ...
  - 위 규칙을 다양한 조건으로 테스트한다.
    ...
    - 이러한 중복 코드는 나중에 대문자를 검사하는 로직이 변경되거나, `one_upper_case_rule()`함수의 시그니처가 변경될 경우 모든 부분을 찾아서 수정해야 한다는 문제가 있다.
  ...
  - 유닛 테스트 프레임워크들은 다양한 입력 값을 테스트할 수 있는 기능을 제공한다.
    - Jest의 경우 `test.each()`가 있으며, pytest의 경우 입력값을 parameterize 하는 기능을 제공한다.
...
- 스텁을 사용하는 이유
  ...
  - 아래와 같은 코드를 테스트해야 한다고 가정해보자.
    ...
    - 이는 테스트의 실행 결과가 테스트를 실행한 날의 요일에 따라 달라질 수 있음을 의미한다.
  ...
  - 위 테스트의 문제점
    ...
    - 테스트 코드나 프로덕션 코드를 변경하지 않는 한 이 윈칙은 유지돼야 한다.
...
- 함수를 이용한 주입 방법
  ...
  - 고차 함수 사용
    ...
    - 고차 함수를 사용하면 새로운 함수를 반환하는 함수를 만들 수 있다.
...
- 객체 지향적으로 의존성을 주입하는 방법
  ...
  - 여러 테스트에 위 클래스를 주입해야 한다면 아래와 같이 클래스의 인스턴스를 만드는 과정을 팩토리 함수로 분리하면 좋다.
