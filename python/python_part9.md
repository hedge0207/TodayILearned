...
- Python은 컴파일 언어 보다는 인터프리터 언어에 가깝다.
  - 좋은 컴파일러는 반복되거나 불필요한 연산을 미리 찾아내어 속도를 높일 수 있다.
    - Java에서는 JIT(Just In Time) 컴파일러를 사용하여 자주 호출되는 메서드(hot spot)를 추적하여 최적화한다.
  - 그러나 인터프리터 언어에 가까운 Python은 위 방식이 불가능하다.
    - Python의 Python 구현체인 PyPy는 Tracing JIT를 지원하여 최적화가 가능하다.
...
- Python의 object model은 비효율적인 메모리 접근을 유발할 수 있다.
  ...
  - 예시
    ...
    - 다른 언어들에서 일반적으로 원시 자료형이라 불리는 integer 등도 Python에서는 하나의 객체이다.
