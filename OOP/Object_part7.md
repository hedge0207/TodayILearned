...
- **리스코프치 치환 원칙(Liskov Substitution Principle, LSP)**
  ...
  - 리스코프 치환 원칙을 위반하는 고전적인 사례
    - "정사각형은 직사각형이다(Squre is-a Rectangle)"가 당연히 참이라고 생각할 것이다.
...
- 생성자도 LSP를 준수해야하는 대상인가

  > 책에서 다루는 내용은 아니다.

  - 일반적으로 생성자 메서드의 경우 LSP를 준수해야하는 대상으로 보지는 않는다.
    - LSP의 "서브타입이 기반 타입을 대체할 수 있어야 한다"는 원칙에서의 대체 가능성은 일반적으로 이미 생성된 객체를 다룰 때를 전제로하기 때문이다.
    - 따라서 객체를 생성하는 생성자 메서드의 경우 LSP를 준수해야하는 대상으로 보지는 않는다.
  - 그러나, 만약 객체의 생성이 팩토리나 프레임워크를 통해 이루어질경우, 혹은 의존성 주입 등을 통해서 이루어질 경우에는 생성자 역시 LSP를 준수해야 한다.
    - 즉 일반적인 경우 LSP는 "행동"의 대체 가능성에 초점을 맞추지만, 동적 생성 패턴에서는 생성 시점의 대체 가능성도 요구된다.
    - 즉 클라이언트가 서브클래스를 다형적으로 생성한다면 생성 역시 계약의 대상이 된다.
