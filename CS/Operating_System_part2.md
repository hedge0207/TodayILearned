...
- CPU migration
  ...
  - CPU migraition은 하나의 프로세스가 한 CPU core에서 다른 CPU core로 이동하여 실행되는 현상을 의미한다.
    - 앞서 살펴본 context switching은 한 CPU 코어 내에서 실행 중인 프로세스를 다른 프로세스로 교체하는 작업이며, 실행 흐름이 전환된다는 점에서 유사하다.
    - 그러나 context switching은 프로세스 간 전환, CPU migration은 코어 간 이동이라는 점에서 개념적으로 구분된다.
  - 주로 유휴 상태(idle) 상태인 CPU core에 프로세스를 할당하기 위해 발생한다.
    - 이런 식으로 로드 밸런싱을 수행하여 CPU core들 사이에 부하가 고르게 분산되도록 한다.
  - 아래와 같이 CPU migration의 발생을 확인할 수 있다.
    - psr을 통해 해당 프로세스가 실행 중인 논리 코어의 번호를 확인할 수 있으며, 아래 명령어를 통해 반복 확인하면 psr 값이 변경되는 것을 확인할 수 있다.

  ```bash
  $ ps -o pid,psr -p <PID>
  ```

  - 프로세스를 다른 CPU core로 이동시키는 작업은 비용이 많이 들기에 비용과 시스템 전체 효율을 비교하여 migration 여부를결정한다.
    - CPU 간 L1/L2 캐시는 공유되지 않기 때문에 새 코어에서는 데이터 캐시가 없고 다시 불러와야 한다.
    - 주소 변환 캐시(TLB)도 CPU마다 다르므로 새 코어에서 재생성 필요하다.
