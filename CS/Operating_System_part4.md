...
- 세그먼테이션 기법
  ...
  - 가변 분할 방식을 이용한 가상 메모리 관리 기법이다.
    ...
    - 페이징 기법은 메모리를 같은 크기의 페이지로 분할하기에 매핑 테이블에 크기 정보룰 유지할 필요가 없으나, 세그먼테이션 기법에서는 프로세스의 크기에 따라 메모리를 분할하기 때문에 매핑 테이블에 크기 정보를 포함한다.
    - 세그먼테이션 역시 물리 메모리가 부족할 경우 스왑 영역을 사용한다.
    - 세그먼테이션 기법은 가변 분할 방식을 기본으로 하므로 가변 분할 방식의 장점과 단점을 모두 가지고 있다.
    - 즉, 메모리를 프로세스 단위로 관리하기에 세그먼테이션 테이블이 작고 단순하다는 장점이 있으나, 외부 단편화로 인해 물리 메모리 관리가 복잡하다는 단점이 있다.
  - 아래는 세그먼테이션 테이블의 예시로, `I`(Invalid)는 해당 프로세스가 스왑 영역에 있다는 것을 의미한다.
